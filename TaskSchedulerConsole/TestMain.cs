...
namespace TestApp {
	...
	/// add this to the post-build command line event if you need to execute this under admin privileges on vista:
	/// mt.exe -manifest "$(ProjectDir)app.manifest" -outputresource:"$(TargetDir)$(TargetName).exe;#1"
	/// </remarks>
	class TestApp {
		...
		{
			ReadOnlyTask rt = new ReadOnlyTask(t);
			Console.WriteLine(rt.Name);
			foreach (Trigger tr in rt.Triggers)
			{
		...
		{
			ScheduledTasksCollection coll = new ScheduledTasksCollection().Load();
			foreach (ReadOnlyTask rt in coll)
			{
				Console.WriteLine(rt.Name);
			}
		}
